...
//   return (
//     <div className="bg-[#63B2AE] dark:bg-black absolute overflow-hidden inset-0 pointer-events-none">
//       {notes.map((note) => (
//         <div
//           key={note.id}
...
const noteIcons = [BsMusicNoteBeamed, BsMusicNote, GiMusicalNotes, FaMusic, GiMicrophone, GiSpeaker];

const noteData = Array.from({ length: 15 }, (_, i) => ({
  id: i,
  icon: noteIcons[i % noteIcons.length], // Directly assigning the component
  size: 10 + (i % 10) + 10,
  top: (i * 7) % 100,
  left: (i * 13) % 100,
  opacity: 0.1 + (i % 5) / 10,
...
export const MusicBackground = () => {
  const [notes, setNotes] = useState(noteData); // Directly initializing with noteData

  return (
    <div className="bg-[#63B2AE] dark:bg-black absolute overflow-hidden inset-0 pointer-events-none">
      {notes.map((note) => {
        const IconComponent = note.icon; // Retrieve the icon component
        return (
          <div
            key={note.id}
            className="absolute text-primary-light animate-float"
